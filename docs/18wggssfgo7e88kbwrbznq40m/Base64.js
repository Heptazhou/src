...
;(function () {
	...
	var encodeFromBytes = function (bytes) {
		...
		if (remain === 1) {
			...
			v1 = bytes[i++]
			v2 = bytes[i]
			base64Str += BASE64_ENCODE_CHAR[v1 >>> 2] + BASE64_ENCODE_CHAR[((v1 << 4) | (v2 >>> 4)) & 63] + BASE64_ENCODE_CHAR[(v2 << 2) & 63] + "="
		}
		return base64Str
	}
	...
	if (NODE_JS) {
		...
		atob = function (base64Str) {
			...
			for (var i = 0, count = (length >> 2) << 2; i < count; ) {
				...
				v3 = BASE64_DECODE_CHAR[base64Str.charAt(i++)]
				v4 = BASE64_DECODE_CHAR[base64Str.charAt(i++)]
				str += String.fromCharCode(((v1 << 2) | (v2 >>> 4)) & 255) + String.fromCharCode(((v2 << 4) | (v3 >>> 2)) & 255) + String.fromCharCode(((v3 << 6) | v4) & 255)
			}
			...
			if (remain === 2) {
				...
				v2 = BASE64_DECODE_CHAR[base64Str.charAt(i++)]
				v3 = BASE64_DECODE_CHAR[base64Str.charAt(i++)]
				str += String.fromCharCode(((v1 << 2) | (v2 >>> 4)) & 255) + String.fromCharCode(((v2 << 4) | (v3 >>> 2)) & 255)
			}
			return str
		}
		...
		utf8Base64Encode = function (str) {
			...
			for (var i = 0; i < str.length; i++) {
				...
				if (charcode < 0x80) {
					...
					result += String.fromCharCode(0xe0 | (charcode >> 12)) + String.fromCharCode(0x80 | ((charcode >> 6) & 0x3f)) + String.fromCharCode(0x80 | (charcode & 0x3f))
				} else {
					charcode = 0x10000 + (((charcode & 0x3ff) << 10) | (str.charCodeAt(++i) & 0x3ff))
					result += String.fromCharCode(0xf0 | (charcode >> 18)) + String.fromCharCode(0x80 | ((charcode >> 12) & 0x3f)) + String.fromCharCode(0x80 | ((charcode >> 6) & 0x3f)) + String.fromCharCode(0x80 | (charcode & 0x3f))
				}
			}
			return btoa(result)
		}
